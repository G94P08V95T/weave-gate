...
pub struct RequestHandlerOpts {
    ...
    /// HTTP client for reverse proxy upstream requests.
    #[cfg(feature = "proxy")]
    pub proxy_client: Option<std::sync::Arc<crate::proxy::HttpClient>>,
    /// Proxy runtime settings (pool, ordering).
    #[cfg(feature = "proxy")]
    pub proxy_settings: Option<crate::settings::ProxySettings>,
    /// When true, evaluate proxy rules before redirects/rewrites.
    #[cfg(feature = "proxy")]
    pub proxy_first: bool,
    /// Limits concurrent protocol-upgrade tunnels when configured.
    #[cfg(feature = "proxy")]
    pub upgrade_tunnel_semaphore: Option<std::sync::Arc<tokio::sync::Semaphore>>,
}
...
impl RequestHandler {
    #[cfg(feature = "proxy")]
    async fn try_reverse_proxy(
        &self,
        req: &mut Request<Body>,
        remote_addr: Option<SocketAddr>,
    ) -> Option<Result<Response<Body>, Error>> {
        let result = crate::proxy::pre_process(&self.opts, req, remote_addr).await?;
        let resp = match result {
            Ok(resp) => resp,
            Err(err) => {
                tracing::debug!("proxy upstream failed: {err:#}");
                return Some(
        ...
        match cors::post_process(&self.opts, req, resp) {
            Ok(r) => Some(Ok(r)),
            Err(e) => Some(Err(e)),
        }
    }
    ...
    pub fn handle<'a>(
        ...
        async move {
            ...
            let result: Result<Response<Body>, Error> = async {
                ...
                // Reverse proxy (early path when proxy-first is enabled)
                #[cfg(feature = "proxy")]
                if self.opts.proxy_first {
                    if let Some(result) = self.try_reverse_proxy(req, remote_addr).await {
                        return result;
                    }
                }
                ...
                // Reverse proxy (default: after redirects/rewrites)
                #[cfg(feature = "proxy")]
                if !self.opts.proxy_first {
                    if let Some(result) = self.try_reverse_proxy(req, remote_addr).await {
                        return result;
