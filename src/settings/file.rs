...
#[derive(Debug, Serialize, Deserialize, Clone)]
#[serde(rename_all = "kebab-case")]
/// Reverse proxy rule: forward matching requests to an upstream backend.
pub struct Proxy {
    /// Optional service name used in logs (e.g. `api-service`).
    pub name: Option<String>,
    /// Optional `Host` header value to match (port is ignored). Matches all hosts when omitted.
    pub host: Option<String>,
    /// Glob pattern matched against the request path (e.g. `/api/**`).
    pub source: String,
    /// Single upstream base URL. Use `target` or `targets`, not both.
    pub target: Option<String>,
    /// Multiple upstream instances for round-robin load balancing. Use `target` or `targets`, not both.
    pub targets: Option<Vec<String>>,
    /// Optional path prefix removed before appending the request path to the upstream URL.
    pub strip_prefix: Option<String>,
}

#[cfg(feature = "proxy")]
/// TLS settings for outbound HTTPS (`[advanced.proxy-tls]`).
#[derive(Debug, Serialize, Deserialize, Clone)]
#[serde(rename_all = "kebab-case")]
pub struct ProxyTls {
    /// PEM file with one or more CA certificates (e.g. mkcert: `$(mkcert -CAROOT)/rootCA.pem`).
    pub ca_file: Option<PathBuf>,
    /// Additional PEM CA files merged into the trust store.
    pub ca_files: Option<Vec<PathBuf>>,
    /// When true, only trust `ca-file` / `ca-files` (no public WebPKI roots).
    pub disable_webpki_roots: Option<bool>,
}

#[cfg(feature = "proxy")]
/// Control-plane routes bootstrap settings (`[advanced.routes-bootstrap]`).
#[derive(Debug, Serialize, Deserialize, Clone)]
#[serde(rename_all = "kebab-case")]
pub struct RoutesBootstrap {
    /// Control plane URL (GET). Query params `client` and `weavegate` are appended.
    pub url: String,
    /// Environment variable name holding the Bearer token (optional).
    pub token_env: Option<String>,
    /// HTTP request timeout in seconds (default: 5).
    pub timeout_secs: Option<u64>,
    /// Path to write/read the last successful JSON snapshot.
    pub cache_path: Option<PathBuf>,
    /// Behavior when the HTTP fetch fails.
    pub on_failure: Option<RoutesBootstrapOnFailure>,
    /// Value for the `client` query parameter (optional).
    pub client_id: Option<String>,
}

#[cfg(feature = "proxy")]
/// Failure policy for routes bootstrap.
#[derive(Debug, Serialize, Deserialize, Clone, Copy, Default, PartialEq, Eq)]
#[serde(rename_all = "kebab-case")]
pub enum RoutesBootstrapOnFailure {
    /// Fetch → cache file → TOML proxies (default).
    #[default]
    CacheThenFile,
    /// Abort startup if fetch and cache are unavailable.
    Fail,
    /// Do not call the control plane; use TOML proxies only.
    FileOnly,
}
...
pub struct Advanced {
    ...
    #[cfg(feature = "proxy")]
    /// Reverse proxy rules for upstream API backends
    pub proxies: Option<Vec<Proxy>>,
    #[cfg(feature = "proxy")]
    /// Startup HTTP fetch of proxy rules from a control plane (v1 API).
    pub routes_bootstrap: Option<RoutesBootstrap>,
    #[cfg(feature = "proxy")]
    /// TLS trust for HTTPS upstreams and routes bootstrap (e.g. mkcert `rootCA.pem`).
    pub proxy_tls: Option<ProxyTls>,
    #[cfg(feature = "proxy")]
    /// Max idle HTTP connections per upstream host in the proxy client pool.
    pub proxy_pool_max_idle_per_host: Option<usize>,
    #[cfg(feature = "proxy")]
    /// Idle timeout in seconds for pooled upstream connections.
    pub proxy_pool_idle_timeout_secs: Option<u64>,
    #[cfg(feature = "proxy")]
    /// When true, run reverse proxy matching before redirects/rewrites (after auth/CORS).
    pub proxy_first: Option<bool>,
    #[cfg(feature = "proxy")]
    /// Max concurrent protocol-upgrade tunnels (WebSocket, etc.). `0` = unlimited.
...
impl Settings {
    ...
    pub fn read(config_file: &Path) -> Result<Settings> {
        ...
        if ext.is_none() || ext.unwrap().is_empty() || ext.unwrap().ne("toml") {
            bail!("configuration file should be in toml format. E.g `weavegate.toml`");
        }
