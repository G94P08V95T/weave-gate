...
//! # WeaveGate
//!
//! WeaveGate is a cross-platform, high-performance frontend gateway for static files, reverse proxy,
//! and round-robin load balancing across multiple upstream instances.
//!
//! **Considerations:**
//!
//! This crate was published to make it possible for users to embed WeaveGate with ease instead of working around it via forks.
//! For example, allowing users to turn off default features like `tls` and so on.
//!
//! **However**, because the library is highly coupled with the WeaveGate binary project at this point,
//! users might be limited by the exposed APIs, implementations, missing functionality, or dependencies.
//!
//! In the future, we will eventually plan to make the WeaveGate library independent from the binary project.
//!
...
//!
//! This is the official WeaveGate crate.
//!
//! ## Overview
//!
//! **WeaveGate** weaves static web assets and API traffic through a single origin: reverse proxy,
//! round-robin upstream selection, and long-lived connection tunneling (WebSocket / SSE).
//!
...
//! `all` | Activates all features including the default and experimental ones. E.g. this feature is used when building WeaveGate binaries.
